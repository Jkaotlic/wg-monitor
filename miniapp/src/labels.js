...
const INCIDENT_COPY = {
  ...
  // ушёл на запасные, запасные недоступны, запасные не снялись рядом с
  // отвечающим своим. Карточка знает только имя проверки, поэтому «что» не
  // утверждает, что свой сервер молчит, а «почему» честно называет все три --
  // третий словами бота.
  resolver_guard: {
    what: 'Неполадка с DNS-серверами роутера',
    why: 'Чаще всего свой DNS-сервер не отвечает и роутер временно перешёл на запасные DNS-серверы — сайты открываются, а когда свой снова ответит, роутер вернётся на него сам. Если и запасные недоступны, сайты по имени могут не открываться. Бывает и так, что свой DNS-сервер отвечает, а запасные DNS-серверы не снялись — сайты открываются, но часть запросов идёт мимо фильтров; роутер убирает их сам.',
  },
}
