...
const INCIDENT_COPY = {
  ...
  // Бот различает три исхода по details.reason (alerts/format.go): роутер
  // ушёл на запасные, запасные недоступны, запасные не снялись рядом со
  // своим. Карточка знает только имя проверки, поэтому «что» не утверждает,
  // что свой сервер молчит, а «почему» честно называет все три -- третий
  // словами бота. Про третий не сказано «свой отвечает»: в окне удержания
  // запасные остаются как раз тогда, когда свой молчит.
  resolver_guard: {
    what: 'Неполадка с DNS-серверами роутера',
    why: 'Чаще всего свой DNS-сервер не отвечает и роутер временно перешёл на запасные DNS-серверы — сайты открываются, а когда свой снова ответит, роутер вернётся на него сам. Если и запасные недоступны, сайты по имени могут не открываться. Бывает и так, что роутер снова на своём DNS-сервере, а запасные DNS-серверы не снялись — сайты открываются, но часть запросов идёт мимо фильтров; роутер убирает их сам.',
  },
}
