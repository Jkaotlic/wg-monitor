...
//
// Сверка с живым роутером (20.08.2026) показала: route_rebind на роутере с
// моделью политик переписывает политические правила в привязку к интерфейсу,
// когда исходный туннель помечен default_route -- а на домашнем роутере им
// помечены все четыре. Кнопка «перенести правила» в этом случае меняет саму
// модель маршрутизации, о чём человеку никто не говорил.
//
// Поэтому перенос предлагается только там, где у туннеля есть СВОИ правила:
// они привязаны к интерфейсу, и перенос делает ровно то, что обещает.
// Правила, приехавшие через политику, переносятся сменой главного звена
