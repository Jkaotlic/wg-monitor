...
// rulesThrough -- сколько всего идёт через этот VPN-туннель. Считается ровно так же,
// как в её собственной строке (tunnelRows → total): в списке ниже видны
// только явно названные правила, а через VPN-туннель идут ещё и те, что приходят
...
export function routingVerdict(snapshot) {
  ...
  if (snapshot?.singbox_router?.enabled) {
    return {
      ...
      detail:
        'Для каждого адреса отдельно. Ни правила HydraRoute Neo — движка умной раздельной маршрутизации, ни статические маршруты на этом роутере не действуют: картинка ниже показывает их, но не то, куда реально идёт трафик.',
    }
  }
  ...
  if (carrying.length === 1) {
    ...
    // VPN-туннель идёт только прописанное, остальное -- напрямую через провайдера.
    ...
    return {
      ...
      title: n > 0
        ...
        : `VPN-туннель «${t.name || t.id}» готов, но пуст`,
  ...
  if (carrying.length > 1) {
    ...
    return {
      mode: 'unknown',
      partial,
      title: 'Главный VPN-туннель не определён',
  ...
  if (claiming.length > 0) {
    ...
    return {
      ...
      detail: `Основным настроен ${claiming.length > 1 ? 'сразу несколько VPN-туннелей' : 'VPN-туннель'} «${names}», но ${claiming.length > 1 ? 'все они выключены' : 'он выключен'} — трафик уходит через провайдера.`,
    }
  }
  if (tunnels.length > 0) {
    return {
  ...
  return {
    ...
    detail: 'Роутер не сообщил ни одного VPN-туннеля. Соберите снимок заново.',
  }
}
...
// lineNameByInterface -- имя VPN-туннеля по имени его интерфейса. Регистр в снимке
// приходит как попало («OpkgTun10» в правиле против «opkgtun10» у туннеля), и
// узнавать туннель это мешать не должно.
...
// Заголовок группы правил -- имя VPN-ТУННЕЛЯ, через который они идут.
//
// Раньше здесь стояло имя интерфейса («OpkgTun10»): системное имя, которого
// человек нигде не видел. В приложении VPN-туннель зовётся своим именем, и
...
// Движок обхода блокировок и одна из политик роутера зовутся почти одинаково
// -- "HydraRoute". В строке "2 правила (2 через HydraRoute)" под политикой RU
// это читалось как "правила RU уходят в политику HydraRoute", то есть прямо
// наоборот. Поэтому движок здесь зовётся полным именем -- "HydraRoute Neo":
// «Neo» отличает его от политики, а сокращение «HR Neo» человеку не говорило
// ничего. Пояснение «движок умной раздельной маршрутизации» ставится там,
// где движок назван в предложении; в счётчиках и подписях -- только имя.
const HR_NEO = 'HydraRoute Neo'
...
// Строка под VPN-туннелем. Механизм («через политику», «через HydraRoute Neo») и цепочка --
// словарь движка маршрутизации: он уехал в «Подробности», а здесь остаётся
// то, что человек понимает без объяснений, -- сколько всего через него идёт.
export function tunnelRuleSummary(row) {
  const total = row?.total ?? 0
  if (total === 0) return 'через него пока ничего не идёт'
  ...
  return `${rulesCount(total)} ${one ? 'идёт' : 'идут'} через него`
}
...
// Можно ли предлагать «Перенести всё» у этого туннеля.
//
// Только там, где у туннеля есть СВОИ правила: они привязаны к интерфейсу, и
// перенос делает ровно то, что обещает. Правила, приехавшие через политику,
// привязаны цепочкой самой политики -- их «переносят» сменой главного звена
// («Сделать главным»), а не этой кнопкой.
//
// Сверка с живым роутером (20.08.2026) показала, зачем это правило: агент
// тогда переписывал политические правила в привязку к интерфейсу, если
// исходный туннель помечен default_route (а на домашнем роутере им помечены
// все четыре) -- то есть «перенести правила» меняло саму модель
// маршрутизации. В агенте это починено, и теперь перенос такие правила
// пропускает: кнопка над ними была бы обещанием без действия.
export function canRebindTunnel(row) {
