...
// регистре, с обычным дефисом вместо U+2011/U+2010 -- скопированное из
// текста, а роутер другого дефиса в имени не примет. www. не трогается --
// это другое имя, и правила бывают на нём.
export function normalizeSiteInput(raw) {
  let s = plainHyphens(raw).trim().toLowerCase()
  s = s.replace(/^https?:\/\//, '')
  s = s.split(/[/?#]/, 1)[0]
  s = s.replace(/:\d*$/, '')
