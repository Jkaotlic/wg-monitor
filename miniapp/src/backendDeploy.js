...
export function backendDeploySheetText(target) {
  return {
    ...
    body: `Сервер скачает ${target} и перезапустится. Пока он перезапускается, приложение не отвечает — обычно минуту-две. Откатить бэкенд из приложения нельзя. Наберите версию точно: ${target} — с буквой v, как написано.`,
  }
}
