...
// недоступны, запасные не снялись рядом со своим. Поэтому «что»
// не утверждает, что свой сервер молчит, а «почему» честно называет все три.
it('dns_split подписана «Раздельный DNS»', () => {
  expect(checkLabel('dns_split')).toBe('Раздельный DNS')
})
...
describe('resolver_guard говорит «Свой DNS-сервер»', () => {
  ...
  it('в карточке тревоги -- что случилось и чем грозит', () => {
    ...
    // Запасные не снимаются и в окне удержания, когда свой как раз молчит:
    // «свой DNS-сервер отвечает» было бы неправдой (тот же текст бота).
    expect(copy.why).not.toContain('свой DNS-сервер отвечает')
