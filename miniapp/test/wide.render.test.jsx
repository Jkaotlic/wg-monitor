...
vi.mock('../src/screens/SettingsScreen.jsx', () => ({ SettingsScreen: () => <div class="stub stub-settings">настройки</div> }))
...
describe('широкая раскладка в браузере', () => {
  ...
  it('выбор роутера в колонке -- шапка с вкладками сверху', async () => {
    ...
    expect([...root.querySelectorAll('.main-tab')].map((b) => b.textContent)).toEqual(['Сейчас', 'VPN-туннели', 'Проверки', 'Что было'])
  ...
  it('оверлей -- в основной области, вкладка закрывает его', async () => {
    ...
    await flush()
    expect(root.querySelector('.stub-settings')).toBe(null)
    expect(window.location.search).toBe('?router=2&tab=events')
    cleanup(root)
  })

  it('шестерёнка открывает настройки, «Парк» -- обслуживание и подсвечивается', async () => {
    setWide(true)
    const root = await mountAt('/dashboard/?router=2')
    await act(async () => root.querySelector('.main-gear').click())
    await flush()
