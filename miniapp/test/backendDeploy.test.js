...
describe('предложение раскатки', () => {
  ...
  it('текст листа', () => {
    expect(backendDeploySheetText('v0.36.0')).toEqual({
      ...
      body: 'Сервер скачает v0.36.0 и перезапустится. Пока он перезапускается, приложение не отвечает — обычно минуту-две. Откатить бэкенд из приложения нельзя. Наберите версию точно: v0.36.0 — с буквой v, как написано.',
    })
  })
