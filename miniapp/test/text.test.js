...
describe('keepTogether', () => {
  ...
  // Термин пишется только так: «VPN-туннель». Строчное «vpn-туннель» -- уже
  // чьё-то имя (идентификатор, показанный без «ёлочек»), и его не трогают.
  it('термин -- с учётом регистра: строчное «vpn-туннель» не трогает', () => {
    expect(keepTogether('интерфейс vpn-туннель-1')).toBe('интерфейс vpn-туннель-1')
    expect(keepTogether('Vpn-туннель и VPN-Туннель')).toBe('Vpn-туннель и VPN-Туннель')
    expect(keepTogether('vpn-туннель-1 и VPN-туннель')).toBe(`vpn-туннель-1 и VPN${NBH}туннель`)
  })
