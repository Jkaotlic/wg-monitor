...
MEMORY_FILE = pathlib.Path.home() / '.claude/projects/C--Users-Anex/memory/host_keenetic.md'
...
CMDS = [
    ...
    ('echo "--- ip route get 89.125.101.122 ---"; ip route get 89.125.101.122 2>&1', 'rget-amnezia'),
    ('echo "--- ip route get 89.125.101.122 oif nwg0 ---"; ip route get 89.125.101.122 oif nwg0 2>&1', 'rget-amnezia-oif'),
    ...
    ('echo "--- nslookup wgmonitor.jkaotlic.duckdns.org ---"; nslookup wgmonitor.jkaotlic.duckdns.org 2>&1 | head -10; echo rc=$?', 'ns-backend'),
    ('echo "--- nslookup www.youtube.com ---"; nslookup www.youtube.com 2>&1 | head -10; echo rc=$?', 'ns-youtube'),
    ('echo "--- ping 89.125.101.122 (no -I) 3 pkts ---"; ping -c 3 -W 2 89.125.101.122 2>&1', 'ping-amnezia'),
    ('echo "--- ping -I nwg0 89.125.101.122 3 pkts ---"; ping -c 3 -W 2 -I nwg0 89.125.101.122 2>&1', 'ping-amnezia-nwg0'),
    ...
    ('echo "--- curl direct to 89.125.101.122:8443 (Amnezia AGH if up) ---"; curl -sS --max-time 5 --interface nwg0 -k https://89.125.101.122/ 2>&1 | head -5; echo rc=$?', 'curl-amnezia-https'),
    ('echo "--- nc -vz 89.125.101.122 443 (no bind) ---"; nc -vz -w 5 89.125.101.122 443 2>&1; echo rc=$?', 'nc-amnezia'),
