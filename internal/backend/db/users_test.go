...
func TestUpdateDeployInfoRoundTripsPortableWizardMetadata(t *testing.T) {
	d := newTestDB(t)
	if _, err := d.Users().Insert("alyaba", "tok", "1.1.1.1", "awg11"); err != nil {
		t.Fatal(err)
	}
	if err := d.Users().UpdateDeployInfo("alyaba", DeployInfo{
		SSHHost:             "192.168.31.1",
	...
	if got.LastDeploy == nil || *got.LastDeploy != "2026-05-24T10:20:30Z" ||
		...
		got.AWGMURL == nil || *got.AWGMURL != "https://alyaba.keenetic.pro" ||
...
func TestUpdateDeployInfoPreservesDeployIdentityWhenMissing(t *testing.T) {
	d := newTestDB(t)
	if _, err := d.Users().Insert("puzirek", "tok", "1.1.1.1", "awg11"); err != nil {
		t.Fatal(err)
	}
	if err := d.Users().UpdateDeployInfo("puzirek", DeployInfo{
		SSHHost:             "192.168.1.1",
		SSHPort:             222,
		SSHUser:             "root",
		Arch:                "arm64",
	...
	got, err := d.Users().GetByNickname("puzirek")
	if err != nil {
		t.Fatal(err)
	}
