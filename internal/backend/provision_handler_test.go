...
func TestDashboardRepairReinstall_SecondConcurrentCallRejected409AndNoReMint(t *testing.T) {
	...
	if err := database.Users().UpdateDeployInfo("bronya", db.DeployInfo{
	...
	rec1 := postProvisionJSON(t, mux, http.MethodPost, "/v1/dashboard/agents/bronya/repair", body)
	if rec1.Code != http.StatusAccepted {
		t.Fatalf("first status = %d, body=%s", rec1.Code, rec1.Body.String())
	}
	var resp1 dashboardJobStartResp
...
func TestDashboardRepairRepoint_BuildsRepointJob(t *testing.T) {
	...
	rec := postProvisionJSON(t, mux, http.MethodPost, "/v1/dashboard/agents/bronya/repair", body)
	if rec.Code != http.StatusAccepted {
		t.Fatalf("status = %d, body=%s", rec.Code, rec.Body.String())
	}
	var resp dashboardJobStartResp
...
func TestDashboardRepairReinstall_BuildsBootstrapJobForExistingNick(t *testing.T) {
	...
	beforeHash := tokenHashOf(t, database, "bronya")
