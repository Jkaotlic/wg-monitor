...
func newDeployRouterMux(t *testing.T, awgmURL string) (*db.DB, http.Handler) {
	...
	if _, err := d.Users().UpsertEnrollment("client-g", "tok-client-g-000000000000000000", db.KindStatic, 0); err != nil {
		t.Fatal(err)
	}
	if awgmURL != "" {
		if err := d.Users().UpdateDeployInfo("client-g", db.DeployInfo{AWGMURL: awgmURL}); err != nil {
			t.Fatal(err)
		}
	}
...
func TestDeployRouterBuildsInstallJobWithTransientCreds(t *testing.T) {
	...
	beforeHash := tokenHashOf(t, d, "client-g")
	...
	runAWGMInstallJob = func(_ context.Context, _ string, job awgmInstallJob) (string, error) {
		captured = job
		return "install bootstrap complete for client-g at v0.13.8 (arm64)\n", nil
	}
	t.Cleanup(func() { runAWGMInstallJob = origRun })

	rec := postDeployRouter(t, h, "client-g",
	...
	if captured.RawToken == "" || captured.RawToken == "tok-client-g-000000000000000000" {
	...
	if h := tokenHashOf(t, d, "client-g"); h == beforeHash {
...
func TestDeployRouterRequiresRootPassword(t *testing.T) {
	...
	rec := postDeployRouter(t, h, "client-g", `{"awgm_api_key":"k"}`)
...
func TestDeployRouterRequiresStoredAWGMURL(t *testing.T) {
	...
	rec := postDeployRouter(t, h, "client-g", `{"root_password":"rootpw"}`)
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want 400", rec.Code)
	}
}
