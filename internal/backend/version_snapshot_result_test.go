...
//
// Снимок писался внутри отрисовки панели обслуживания бота, а она запускалась
// только тогда, когда команду поставил человек нажатием кнопки. Суточный опрос
// ставит команду сам, и её ответ до записи не доезжал вовсе: поллер работал бы
// вхолостую, а версия HydraRoute Neo и доступная прошивка так и не появлялись
// бы в базе.
//
...
func TestCmdResultVersionAuditWritesSnapshotWithoutViewer(t *testing.T) {
	...
	// Команда заведена поллером: в очереди она есть, а человека, который ждал
	// бы её итог на экране, нет вовсе.
