(function () {
  ...
  async function deployBackend() {
    ...
    // Единственная кнопка в панели, которая перезапускает сам бэкенд -- и до
    // сих пор она была единственной же без подтверждения: обновление
    // отдельного роутера спрашивает через модальное окно, а обновление
    // бэкенда уходило по первому клику. Во время перезапуска парк не
    // отчитывается и команды не доходят, так что случайный клик стоит
    // дороже любого другого в этой панели.
    if (!window.confirm(
      "Обновить бэкенд до " + version + "?\n\n" +
      "Бэкенд перезапустится: на время перезапуска роутеры не смогут отчитаться, " +
      "а команды и мини-апп будут недоступны. Обновление агентов это не затрагивает."
    )) return;
