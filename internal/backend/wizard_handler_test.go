...
func TestWizardPut_204Updates(t *testing.T) {
	...
	h := wizardPutAgentHandler(Deps{DB: d})
	body := `{"ssh_host":"10.0.0.1","ssh_port":222,"ssh_user":"root","arch":"linux-mips","last_deployed_version":"v0.10.3","ring":"beta","pending_version":"v0.11.0","pending_since":"2026-05-19T10:00:00Z","last_deploy":"2026-05-24T10:20:30Z","deploy_mode":"awgm","awgm_url":"https://client-a.keenetic.example","awgm_auth":"api-key","expected_mac":"aabbccddeeff"}`
...
func TestPublicHostChecksRejectTrailingDotLocalHosts(t *testing.T) {
	for _, host := range []string{"localhost.", "127.0.0.1.", "10.0.0.1.", "192.168.0.87."} {
...
func TestWizardDeployDoesNotEnqueueWhenPendingUpdateFails(t *testing.T) {
	dbPath := t.TempDir() + "/state.db"
	d, err := db.Open(dbPath)
	if err != nil {
		t.Fatal(err)
	...
	if len(sink.enqueued) != 0 {
		t.Fatalf("pending update failed but command was enqueued: %+v", sink.enqueued)
	}
}
...
func TestWizardCommandDispatchRejectsUnsafeCommand(t *testing.T) {
	...
	for _, body := range []string{
		...
		t.Run(body, func(t *testing.T) {
			sink := &fakeCmdSink{}
			h := wizardCommandHandler(Deps{DB: d, CommandSink: sink})
			req := httptest.NewRequest(http.MethodPost, "/v1/wizard/agents/client-b/commands", strings.NewReader(body))
			req.Header.Set("Content-Type", "application/json")
			req.SetPathValue("nickname", "client-b")
			rec := httptest.NewRecorder()
			h.ServeHTTP(rec, req)
