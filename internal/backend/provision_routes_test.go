...
func TestDashboardRepair_WiredThroughRealMux(t *testing.T) {
	...
	rec := doDashboardJSON(t, h, http.MethodPost, "/v1/dashboard/agents/client-g/repair",
...
func TestDashboardProvisionRoutes_RequireDashboardAuth(t *testing.T) {
	...
	repairReq := httptest.NewRequest(http.MethodPost, "/v1/dashboard/agents/client-g/repair", strings.NewReader(`{"mode":"repoint","root_password":"x"}`))
