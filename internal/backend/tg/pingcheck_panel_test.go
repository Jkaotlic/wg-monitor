...
func TestPingCheckPanelText_AliveAndDead(t *testing.T) {
	n417, n5 := int64(417), int64(5)
	entries := []PingCheckPanelEntry{
		{TunnelID: "awg10", Name: "amst", Status: "alive", PerTunnelEnabled: true, LastLatencyMs: 82, SuccessCount: &n417, FailCount: 0, FailThreshold: 3, RestartCount: 0},
		{TunnelID: "awg11", Name: "fra", Status: "dead", PerTunnelEnabled: true, LastLatencyMs: 0, SuccessCount: &n5, FailCount: 3, FailThreshold: 3, RestartCount: 7},
	}
...
func TestPingCheckPanelText_LongSuccessCount(t *testing.T) {
	n := int64(12500)
	entries := []PingCheckPanelEntry{{TunnelID: "awg10", Name: "amst", Status: "alive", PerTunnelEnabled: true, LastLatencyMs: 50, SuccessCount: &n}}
	got := PingCheckPanelText("router1", true, entries)
...
func TestPingCheckPanelText_OmitsUnknownSuccessCount(t *testing.T) {
	// awg-manager 2.17.2 поля successCount не отдаёт вовсе. Печатать "✓0" --
	// это выдавать отсутствие данных за результат измерения.
	entries := []PingCheckPanelEntry{
		{TunnelID: "awg11", Name: "awg3-work-via-ru1", Status: "alive", PerTunnelEnabled: true, LastLatencyMs: 60, FailCount: 0, FailThreshold: 3},
	}
	got := PingCheckPanelText("client-b", false, entries)
	if strings.Contains(got, "✓") {
		t.Errorf("unknown success count must not be printed:\n%s", got)
	}
	if !strings.Contains(got, "✗0/3") {
		t.Errorf("fail counters must stay:\n%s", got)
	}
}

func TestPingCheckPanelText_PrintsKnownSuccessCount(t *testing.T) {
	n := int64(417)
	entries := []PingCheckPanelEntry{
		{TunnelID: "awg10", Name: "amst", Status: "alive", PerTunnelEnabled: true, LastLatencyMs: 82, SuccessCount: &n, FailCount: 0, FailThreshold: 3},
	}
	got := PingCheckPanelText("client-b", true, entries)
	if !strings.Contains(got, "✓417") {
		t.Errorf("known success count must be printed:\n%s", got)
	}
}
