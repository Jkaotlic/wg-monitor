...
func ComputeUpdates(ctx context.Context, cache *Cache, va wire.VersionAudit) ([]UpdateInfo, []Unknown) {
	...
	// Модуль ядра ни с чем по парку не сравнивается: «нужна перезагрузка» --
	// это расхождение установленного и загруженного модуля (RebootHint). Здесь
	// остаётся единственный честный ответ -- агент про модуль вовсе не сообщает.
	if va.KmodVersion == "" {
...
// RebootHint отвечает по-русски и о последствии. Строится на расхождении
// установленной и загруженной версии модуля ядра (wire.RebootNeeded): модуль
// сменили, ядро держит старый. После перезагрузки версии совпадают, и
// предупреждение гаснет само -- история снимков для этого не нужна.
func RebootHint(installedKmod, loadedKmod string) string {
	if !wire.RebootNeeded(installedKmod, loadedKmod) {
		return ""
	}
	return "Сменился модуль ядра AmneziaWG — VPN-туннели поднимутся после перезагрузки роутера."
}
