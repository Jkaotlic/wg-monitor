...
func TestBuildReviveScriptRewritesBackendURLAndRestarts(t *testing.T) {
	s := buildReviveScript("https://wg.client-c.example")
	for _, want := range []string{
		"NEW='https://wg.client-c.example'",
...
func seedReviveAgent(t *testing.T, d *db.DB, awgmURL string) {
	t.Helper()
	if _, err := d.Users().UpsertEnrollment("client-c", "tok-client-c-000000000000000000", db.KindStatic, 0); err != nil {
		t.Fatal(err)
	}
	if awgmURL != "" {
		if err := d.Users().UpdateDeployInfo("client-c", db.DeployInfo{AWGMURL: awgmURL}); err != nil {
			t.Fatal(err)
		}
	}
}
...
func TestDashboardReviveDrivesRelayWithTransientPassword(t *testing.T) {
	...
	if captured.BaseURL != "https://awg.client-c.example" {
