...
func formatQueuedStatus(action, checkName string) string {
	...
	if action == "diag_now" {
		...
		// без IncludeRestart, так что ни один VPN-туннель не трогается.
		// Раньше отдавался последний готовый отчёт и выдавался за новый;
		// теперь отчёт всегда свежий, и обещать нечего -- перезапуска не будет.
		return "📤 Отправлено роутеру: 📊 Диагностика — роутер проверит себя заново, это до минуты; VPN-туннели при этом не перезапускаются"
	}
