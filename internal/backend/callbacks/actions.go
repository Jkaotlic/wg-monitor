...
func formatQueuedStatus(action, checkName string) string {
	...
	if action == "diag_now" {
		// Агент отдаёт ПОСЛЕДНИЙ готовый отчёт роутера и проверяет заново,
		// только если отчёта ещё нет (runner.DiagNow). Лишь тогда awg-manager
		// гоняет restart_cycle и каждый VPN-туннель моргает -- обещать это при
		// каждом нажатии значило бы пугать тем, чего не будет.
		return "📤 Отправлено роутеру: 📊 Диагностика — роутер пришлёт свой последний отчёт проверки; если отчёта ещё нет, проверит себя заново, и каждый VPN-туннель на секунду перезапустится"
	}
