...
func seedEditAgent(t *testing.T, d *db.DB) {
	t.Helper()
	if _, err := d.Users().UpsertEnrollment("client-g", "tok-client-g-0000000000000000000000", db.KindStatic, 0); err != nil {
		t.Fatal(err)
	}
	if err := d.Users().UpdateDeployInfo("client-g", db.DeployInfo{
		Ring:                "rc",
		DeployMode:          "ssh",
