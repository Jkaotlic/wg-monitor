...
func TestRunner_RouterDoctor_OK(t *testing.T) {
	...
	mux.HandleFunc("/api/system/info", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"success":true,"data":{"version":"2.10.6","activeBackend":"nativewg","routerIP":"192.168.31.1","firmwareVersion":"5.00.C.11.0-0","totalMemoryMB":489,"singbox":{"installed":true,"version":"1.13.8"}}}`))
	})
...
func TestRunner_RouterDoctor_FailsWhenProcessMissing(t *testing.T) {
	...
	exec := func(ctx context.Context, name string, args ...string) ([]byte, error) {
		...
		return []byte("default via 192.168.31.1 dev br0\n"), nil
	}
