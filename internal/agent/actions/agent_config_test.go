...
func TestUpdateAgentConfigPatchesAndPreservesRest(t *testing.T) {
	...
	if full.Backend.URL != "https://wgmonitor.anexaev.crazedns.ru" || full.Backend.Token != "secret-token-123" {
...
func TestUpdateAgentConfigIgnoresNonWhitelistedKeys(t *testing.T) {
	...
	if !strings.Contains(string(raw), "https://wgmonitor.anexaev.crazedns.ru") ||
