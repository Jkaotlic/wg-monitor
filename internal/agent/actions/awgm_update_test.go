...
// UpdateCheck -- первый запрос обязан идти с force=true, повторные (пока
// checking:true) -- с force=false (fix round 1, п.1). Несовпадение -- ошибка,
// чтобы код-нарушитель сразу проваливал тест, а не проходил вслепую.
func (f *fakeAwgUpdate) UpdateCheck(ctx context.Context, force bool) (*awgmgr.UpdateCheck, error) {
	wantForce := f.checkCalls == 0
	if force != wantForce {
		return nil, fmt.Errorf("UpdateCheck(force=%v) на вызове #%d, хотим force=%v", force, f.checkCalls+1, wantForce)
	}
	if f.checkErr != nil {
...
// Первый info-шаг -- базовая версия, которую код читает ДО apply (fix round 1,
// п.2); два следующих -- обрывы связи во время опроса, третий -- успех.
func TestAwgmUpdate_VersionChangesAfterTwoDrops(t *testing.T) {
	cli := &fakeAwgUpdate{
		...
		infos: []infoStep{
			{info: &awgmgr.SystemInfo{Version: "2.19.0+r2"}}, // базовая версия до apply
...
func TestAwgmUpdate_HTTPRefusalPollsUntilVersionChanges(t *testing.T) {
	cli := &fakeAwgUpdate{
		...
		infos: []infoStep{
			{info: &awgmgr.SystemInfo{Version: "2.19.0+r2"}}, // базовая версия до apply
			{info: &awgmgr.SystemInfo{Version: "2.19.1"}},    // видна после обновления
		},
	}
	now, sleep, sleeps := fakeUpdateClock()
	out, err := AwgmUpdate(context.Background(), cli, sleep, now)
...
func TestAwgmUpdate_RefusalButAutoInstallerAlreadyUpdated(t *testing.T) {
	cli := &fakeAwgUpdate{
		...
		infos: []infoStep{
			{info: &awgmgr.SystemInfo{Version: "2.19.0+r2"}}, // базовая версия до apply
			{info: &awgmgr.SystemInfo{Version: "2.19.1", KernelModuleVersion: "3.1.20260906", KernelModuleLoadedVersion: "3.1.20260906"}},
		},
	}
	now, sleep, _ := fakeUpdateClock()
	out, err := AwgmUpdate(context.Background(), cli, sleep, now)
...
// Fix round 1, п.1: "checking" не проходит и за 30с -- это ошибка, а не
// молчаливое решение "уже последняя". Полагаться на половинчатый ответ и
// объявлять «всё актуально» значило бы соврать владельцу, когда демон
// на самом деле всё ещё считает.
func TestAwgmUpdate_CheckingTimesOutAfter30sIsError(t *testing.T) {
	cli := &fakeAwgUpdate{
		checks: []awgmgr.UpdateCheck{{Checking: true, CurrentVersion: "2.19.0+r2"}},
	}
	now, sleep, sleeps := fakeUpdateClock()
	_, err := AwgmUpdate(context.Background(), cli, sleep, now)
	if err == nil || err.Error() != "awg-manager ещё проверяет обновления — повторите через минуту" {
		t.Fatalf("err = %v", err)
	}
	if *sleeps != 10 {
...
// Базовая версия читается из SystemInfo ДО apply -- в том же формате, что и
// опрос после. Здесь версия реально не меняется (оба info-шага совпадают),
// и раньше сравнение с check.currentVersion="2.19.0" ложно решило бы
// updated:true на первом же опросе.
func TestAwgmUpdate_FromUsesSystemInfoNotCheckCurrentVersion(t *testing.T) {
	cli := &fakeAwgUpdate{
		check: awgmgr.UpdateCheck{Available: true, CurrentVersion: "2.19.0"},
		infos: []infoStep{{info: &awgmgr.SystemInfo{Version: "2.19.0+r2"}}},
	}
	now, sleep, sleeps := fakeUpdateClock()
	_, err := AwgmUpdate(context.Background(), cli, sleep, now)
	if err == nil || err.Error() != "awg-manager не вернулся с новой версией за 5 минут" {
		t.Fatalf("err = %v (ложное updated:true из-за разных форматов версии)", err)
	}
	if *sleeps != 60 {
		t.Errorf("опросов %d, хотим 60", *sleeps)
	}
...
func TestActionTimeoutFor_MaintenanceUpdates(t *testing.T) {
	if got := actionTimeoutFor("awgm_update"); got != 420*time.Second {
		t.Errorf("awgm_update budget = %v, want 420s: пятиминутный опрос должен успеть сказать своё", got)
	}
}

// Fix round 1, п.3: бюджет обязан с запасом покрывать весь худший путь --
// цикл checking (30с) + опрос после apply (5 мин) -- иначе actionTimeout
// срубит действие раньше, чем оно успеет сказать своё «не вернулся за 5
// минут», и владелец увидит generic timeout вместо дословного текста.
func TestActionTimeoutFor_AwgmUpdateCoversWorstCase(t *testing.T) {
	got := actionTimeoutFor("awgm_update")
	min := awgmCheckingTimeout + awgmUpdateTimeout + 60*time.Second
	if got < min {
		t.Errorf("awgm_update budget = %v, want >= %v (checking 30с + опрос 5 мин + запас 60с)", got, min)
	}
}
