...
- `del`, `client-a`, `client-i`, and `client-b` were retargeted through AWG Manager terminal.
- `client-e`, `client-g`, and `client-f-old` were queued as pending/offline mobile jobs.
...
- public correctness is `https://wgmonitor.example.com/healthz`, not just a local container health check.
...
Several AWG Manager URLs use names like `awg.client-e.example` or `awg.client-f.example`. Their certificates were wildcard certs for one label only, so Go TLS verification failed for two-label subdomains.

Observed pending-job failures:

- `client-e`: certificate valid for `*.ddns.example`, not `awg.client-e.example`.
- `client-f-old`: certificate valid for `*.ddns2.example`, not `awg.client-f.example`.
...
### 6. `client-g` hostname drift

The deferred job for `client-g` resolved `awg.client-d.example` as NXDOMAIN from the Pi. This may be a wrong hostname, disabled mobile WAN, stale DNS, or HTTP-only route.
...
This forced direct AWG Manager operations for `client-b`.
...
### 8. `client-b` was not offline, but had real tunnel failure

`client-b` agent was alive and reporting to the new backend. The failing checks were:
...
- Ping to `198.51.100.22` from the router had 100% packet loss.
...
`client-b` did not fall off because of the backend migration. The agent path is alive. The remaining fault is the AWG tunnel/server/config path for `awg11`, which also explains DNS failure when DNS policy depends on that tunnel.
...
- `client-e`, `client-g`, and `client-f-old` remain pending/offline or blocked by DNS/TLS issues.
- `client-b` agent is alive, but `awg11` remains enabled/starting without handshake.
