...
## Дополнение контроллера: что выяснилось после протокола

### 1. Вывод протокола был сильнее его собственных данных

Обе пробы permit били в `Bridge1`. Продакшен-путь в мост не целится никогда:
`addTunnelToHydraRoutePolicies` берёт имя интерфейса резолвером из
`/api/routing/policy-interfaces` по псевдонимам **импортированного туннеля**, то есть
всегда обращается к туннельному интерфейсу. Поэтому корректная формулировка —
**permit не проверен на туннеле**, а не «permit не работает». Гипотеза «дело в типе
интерфейса» остаётся живой и непроверенной.

### 2. Чтение политик кешируется, и это чуть не стоило нам ложного провала

В OpenAPI роутера у списка политик есть параметр, которым мы не пользовались:
...
То есть список политик по умолчанию отдаётся из кеша NDMS. Проверка постусловия в
`addTunnelToHydraRoutePolicies` перечитывала именно кешированный путь: успешная запись
могла быть отрапортована как провал, и импортированный туннель объявлен непопавшим в
маршрутизацию, хотя попал. Исправлено в рамках фазы отдельным методом чтения с обходом
кеша, применяемым только для проверки постусловия — принудительный промах кеша на каждом
опросе снимка нагружал бы роутер без нужды.

Заодно это позволило перепроверить сам результат пробы. Сравнение двух чтений
непосредственно после откатов:

```
GET /api/routing/access-policies      → HydraRoute: OpkgTun11(0) → Wireguard0(1) → OpkgTun10(2)
GET /api/access-policies?refresh=true → HydraRoute: OpkgTun11(0) → Wireguard0(1) → OpkgTun10(2)
```

Оба совпали. Значит: роутер действительно чист (подтверждено чтением мимо кеша), и
молчаливый no-op permit для `Bridge1` — не эффект устаревшего кеша, а реальное поведение.

### 3. Глагол выбран верно

`POST /api/access-policies/assign` привязывает к политике **LAN-устройство по MAC**
(`api.AssignDeviceRequest`, параметр `mac`), а `permit` — интерфейс к политике. Наш код
использует `permit`, и это правильный вызов для цепочки интерфейсов. Переводить его на
`assign` нельзя.

### 4. Что осталось выяснить и как

Дешёвый способ закрыть вопрос до живой обкатки фазы C: оператор добавляет интерфейс в
политику `HydraRoute` из **родного веб-интерфейса awg-manager** и смотрит, изменилась ли
цепочка. UI вызывает тот же эндпоинт с тем же `order = len(chain)`, поэтому:

- цепочка изменилась → наш запрос отличается от запроса UI, разница ловится и чинится в коде;
- цепочка не изменилась → ограничение роутера на мостах либо на permit вообще, и шаг
  «сделать главным» фазы C нужно проектировать на другом механизме.

Проба «удалить туннельный интерфейс из цепочки и вернуть обратно» сознательно **не
выполнялась**: все три интерфейса, которые политике интересны, уже состоят в цепочке, и при
ненадёжном permit откат мог не сработать, оставив оператора без резерва.
