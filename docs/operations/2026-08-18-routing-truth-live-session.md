...
### Догадка про `order` проверена и отвергнута

Гипотеза протокола — что permit не применился из-за `order:9` вне длины цепочки —
проверена контроллером отдельной мутацией того же класса (разрешение оператора
покрывает `permit` с `order = len(chain)` и немедленный откат):

```
POST /api/access-policies/permit {"name":"HydraRoute","interface":"Bridge1","order":3}
→ {"success":true,"data":{"ok":true}}

GET /api/routing/access-policies (через 2 с)
→ HydraRoute: OpkgTun11(0) → Wireguard0(1) → OpkgTun10(2)   -- Bridge1 отсутствует
```

`order:3` — это ровно `len(chain)`, то самое значение, которое использует наш
`addTunnelToHydraRoutePolicies`. Результат тот же: `success:true`, цепочка не
изменилась. Значит дело не в `order`.

Разбор собственного веб-интерфейса роутера 2.17.2 показывает, что он вызывает
тот же эндпоинт с тем же значением: в обработчике добавления интерфейса стоит
`t.onpermit(D, e(v).length)` — то есть `order` = текущая длина цепочки. Наш
клиент повторяет поведение UI буква в букву.

**Что остаётся непроверенным.** Безопасного способа доказать, что permit
работает для настоящего туннельного интерфейса, на этом роутере нет: три
интерфейса, которые политике интересны (`OpkgTun11`, `Wireguard0`, `OpkgTun10`),
уже состоят в цепочке, а проверка «удалить и вернуть» опасна ровно потому, что
permit может не сработать — оператор остался бы без резерва. Такую мутацию
контроллер сознательно не выполнял.

Отсюда открытый вопрос к живой обкатке фазы C: применяется ли permit к
интерфейсу-мосту (`Bridge1`) вообще, или дело в типе интерфейса. Дешёвый способ
это выяснить — оператору самому добавить интерфейс в политику `HydraRoute` из
родного веб-интерфейса awg-manager и посмотреть, изменится ли цепочка. Если и
там не изменится — ограничение на стороне роутера, и шаг «сделать главным» из
фазы C нужно будет строить иначе.
