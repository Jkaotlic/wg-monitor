...
- Backend health:
  `https://wgmonitor.example.com/healthz` returned
  `{"status":"ok","version":"v0.13.0-rc106"}`.
- Final component table showed backend and online/static agents on
  `v0.13.0-rc106`; `client-e` and `client-f-old` remain pending until
  next wake; `client-g` still needs metadata repair.
