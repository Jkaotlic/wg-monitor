...
> available this session via the operator's WireGuard tunnel to the home LAN (192.168.0.x).
...
- Reachable over operator WG: router `192.168.0.1` (SSH 222, root — testkeen pw in
  [[feedback_awgmgr_api]] memory), backend VPS `192.168.0.87` (not reachable this session).
