...
возврата ключа (заново пройденных шагов выше на том же файле) или ручной отмены в
мини-аппе; после возврата ключа `Recover` при следующем старте подхватит их как
обычно. Замена ключа на новый (файл существовал, но его перезаписали другим) ведёт
