...
func detectBackendSwapPlan(s *SSH) backendSwapPlan {
	if _, _, rc, _ := s.Run("test -f /home/anex/wg-monitor/bin/wg-monitor-backend && command -v docker >/dev/null 2>&1 && docker inspect wg-monitor-backend >/dev/null 2>&1"); rc == 0 {
		return backendSwapPlan{
			Name:       "docker:wg-monitor-backend",
			RemotePath: "/home/anex/wg-monitor/bin/wg-monitor-backend",
