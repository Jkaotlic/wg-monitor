...
func TestApplyAWGMDeploySuccessIfConfirmedRejectsPendingBackendConfirmation(t *testing.T) {
	ag := &AgentState{
		Nickname:            "bronya",
		DeployMode:          "awgm",
		AWGMURL:             "https://awg.bronya.example",
		LastDeployedVersion: "",
	...
	}, ag, &AWGMSystemInfo{RouterIP: "192.168.31.1"}, "v0.13.0-rc59", "api-key", true, time.Unix(1779693600, 0).UTC())
...
func TestShouldOfferBackendMigrationAfterHostChange(t *testing.T) {
	oldBackend := BackendState{Host: "103.106.1.253", Domain: "wg-old.example"}
	newBackend := BackendState{Host: "89.125.101.122", Domain: "wg-old.example"}
...
func TestShouldOfferBackendMigrationSkipsFreshInstall(t *testing.T) {
	...
	newBackend := BackendState{Host: "89.125.101.122", Domain: "wg-new.example"}
	if shouldOfferBackendMigration(oldBackend, newBackend, 2) {
