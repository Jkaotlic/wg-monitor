...
func TestBuildUpdateTargetsUsesReadableEndpointMetadata(t *testing.T) {
	state := &State{Agents: []AgentState{
		...
		},
		{
			Nickname:   "client-a",
			DeployMode: "awgm",
			AWGMURL:    "https://awg.client-a.example.test",
		},
		{
			Nickname: "testkeen",
			Host:     "192.168.0.1",
	...
	assertTargetDisplay(2, "192.168.0.1:222", "LAN")
}
...
func TestFilterUpdateBlockedKeepsRepairNeededTargetsVisible(t *testing.T) {
	targets := []updateTarget{
		{Label: "agent client-g", IsAgent: true, UpdateBlockedReason: "metadata-gap: сначала sync-vps"},
		{Label: "agent ok", IsAgent: true, NeedsUpdate: false},
	}
	blocked := filterUpdateBlocked(targets)
	if len(blocked) != 1 || blocked[0].Label != "agent client-g" {
...
func TestUpdateTargetReachabilityFailureMessageUsesEndpointLabel(t *testing.T) {
	target := updateTarget{
		Host:            "192.168.0.1",
		Port:            0,
		DisplayEndpoint: "client-g-vpn:222",
	}
	got := updateTargetReachabilityFailureMessage(target)
	if !strings.Contains(got, "client-g-vpn:222") {
		t.Fatalf("failure message should use display endpoint, got %q", got)
	}
	if strings.Contains(got, ":0") || strings.Contains(got, "192.168.0.1:0") {
...
func TestUpdateTargetChoiceLabelShowsEndpoint(t *testing.T) {
	cases := []struct {
		...
		},
		{
			name: "awgm agent",
			t: updateTarget{
				Label:           "agent client-a",
				IsAgent:         true,
				DisplayEndpoint: "AWGM",
			},
			want: "agent client-a (AWGM)",
		},
		{
			name: "metadata gap agent",
			t: updateTarget{
				Label:           "agent client-g",
				IsAgent:         true,
				DisplayEndpoint: "metadata-gap",
			},
			want: "agent client-g (metadata-gap)",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestRefreshInstalledComponentStateMergesLiveBackendAndAgents(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/wizard/agents":
			...
			_, _ = w.Write([]byte(`{"agents":[{"nickname":"client-h","kind":"mobile","thread_id":42,"ssh_host":"10.0.0.2","ssh_port":222,"ssh_user":"root","arch":"mips","last_deployed_version":"v0.14.0-rc1","ring":"canary"}]}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	agent := state.FindAgent("client-h")
	if agent == nil {
		t.Fatal("agent client-h missing after refresh")
	}
...
func TestMobilePullAckTimeoutMarksPending(t *testing.T) {
	state := &State{Agents: []AgentState{{Nickname: "client-h", Kind: "mobile"}}}
	target := updateTarget{
		IsAgent:       true,
		AgentNickname: "client-h",
		Kind:          "mobile",
...
func TestRunPullDeployMobileAckTimeoutReturnsPendingError(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPut && r.URL.Path == "/v1/wizard/agents/client-h":
			pushed = true
...
func TestRunOneUpdateUsesAWGMForAgentWithoutSSHHost(t *testing.T) {
	...
	installAgentAWGMForUpdate = func(_ *State, _ *SecretStore, _ *Downloader, nickname string) error {
		called = true
		if nickname != "client-a" {
			t.Fatalf("nickname=%q, want client-a", nickname)
		}
		return nil
	}
