...
func TestRunPullDeployMobileAckTimeoutReturnsPendingError(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/wizard/agents":
			_, _ = w.Write([]byte(`{"agents":[{"nickname":"carvan","kind":"mobile","last_seen_at":"2026-05-23T06:39:00Z"}]}`))
...
func TestPullDeployReadinessAllowsStaleMobilePendingPath(t *testing.T) {
	...
	target := updateTarget{IsAgent: true, AgentNickname: "carvan", Kind: "mobile"}
	...
	for _, want := range []string{"mobile", "carvan", "stale 8h"} {
