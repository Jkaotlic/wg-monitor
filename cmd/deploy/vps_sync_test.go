...
func TestMergeAgents_PullsPortableAWGMMetadata(t *testing.T) {
	local := []AgentState{{
		Nickname:       "alyaba",
		PreferredIface: "SSTP alyaba",
	}}
	remote := []RemoteAgent{{
		Nickname:    "alyaba",
		LastDeploy:  "2026-05-24T10:20:30Z",
		DeployMode:  "awgm",
		AWGMURL:     "https://alyaba.keenetic.pro",
		AWGMAuth:    "api-key",
	...
	if got.LastDeploy != "2026-05-24T10:20:30Z" || got.DeployMode != "awgm" ||
		got.AWGMURL != "https://alyaba.keenetic.pro" || got.AWGMAuth != "api-key" ||
		got.ExpectedMAC != "aabbccddeeff" {
		t.Fatalf("portable metadata not merged: %+v", got)
	}
	if got.PreferredIface != "SSTP alyaba" {
...
func TestMergeAgents_PreservesLocalExpectedMACOnRemoteConflict(t *testing.T) {
	local := []AgentState{{Nickname: "alyaba", ExpectedMAC: "111111111111"}}
	remote := []RemoteAgent{{Nickname: "alyaba", ExpectedMAC: "222222222222"}}

	merged, _, _, macConflicts := MergeAgents(local, remote)
	if merged[0].ExpectedMAC != "111111111111" {
		t.Fatalf("local expected_mac overwritten: %+v", merged[0])
	}
	if !reflect.DeepEqual(macConflicts, []string{"alyaba"}) {
		t.Fatalf("mac conflicts=%v, want alyaba", macConflicts)
	}
}
...
func TestVPSClientCreateEnrollmentFallsBackToSSHOnTransportError(t *testing.T) {
	...
	c := &VPSClient{
		...
		Fallback: wizardAPIFallbackFunc(func(ctx context.Context, method, path string, body []byte, timeout time.Duration) (int, []byte, error) {
			...
			if req.Nickname != "router4car4" || req.Kind != "mobile" || req.ThreadID != 1126 {
				t.Fatalf("bad fallback request: %+v", req)
			}
			return http.StatusCreated, []byte(`{"nickname":"router4car4","backend_url":"https://wg.example.test","raw_token":"raw-token"}`), nil
		}),
	}
...
func TestVPSClientPushAgentCarriesKindAndThread(t *testing.T) {
	var req RemoteAgent
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/wizard/agents/bronya" {
			http.NotFound(w, r)
			return
		}
...
func TestNewVPSClientWithTimeoutAndDialHostKeepsDomainBaseURL(t *testing.T) {
	c := NewVPSClientWithTimeoutAndDialHost("wgmonitor.example", "secret", 22*time.Second, "83.171.224.125")
	if c == nil {
		t.Fatal("client is nil")
	}
	if c.BaseURL != "https://wgmonitor.example" {
...
func TestNewVPSClientForBackendUsesDomainAndHost(t *testing.T) {
	state := &State{Backend: BackendState{Domain: "wgmonitor.example", Host: "83.171.224.125"}}
