...
func TestDeferredAWGMRecoveryHintGivesCopyPastePath(t *testing.T) {
	...
	got := deferredAWGMRecoveryHint(
		&State{Backend: BackendState{Host: "83.171.224.125", Port: 2202, User: "deployer"}},
		&AgentState{Nickname: "bronya"},
		"/var/lib/wg-monitor/deferred-awgm/bronya.json.token",
		"WG_AGENT_TOKEN_BRONYA",
	)
	for _, want := range []string{
		"WG_AGENT_TOKEN_BRONYA",
		"ssh -p 2202 deployer@83.171.224.125 \"cat /var/lib/wg-monitor/deferred-awgm/bronya.json.token\" | Add-Content -Encoding ascii \"" + secPath + "\"",
...
job_path = os.path.join(work_dir, "bronya.json")
with open(job_path, "w", encoding="utf-8") as f:
    json.dump({
        "nickname": "bronya",
        "kind": "mobile",
