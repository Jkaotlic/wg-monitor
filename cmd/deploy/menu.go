...
func routerMenuItems() []menuItem {
	return []menuItem{
		...
		{Key: "5", Title: "Оживить (redeploy + backend URL)", Help: "когда нажимать: роутер офлайн после смены домена backend; по SSH чинит backend.url, накатывает свежий агент и рестартует"},
	}
}
