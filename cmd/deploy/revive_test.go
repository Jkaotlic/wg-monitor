...
func TestReplaceAgentConfigBackendURLRewritesAndPreservesRest(t *testing.T) {
	out, changed, err := replaceAgentConfigBackendURL(reviveSampleConfig, "https://wg.client-c.example")
	if err != nil {
		t.Fatal(err)
	}
	if !changed {
	...
	if !strings.Contains(out, "url: https://wg.client-c.example") {
	...
	if !strings.Contains(out, `token: "secret-token-123"`) || !strings.Contains(out, "nickname: client-c") ||
...
func TestReplaceAgentConfigBackendURLNoChangeWhenAlreadyCorrect(t *testing.T) {
	_, changed, err := replaceAgentConfigBackendURL(reviveSampleConfig, "https://wgmonitor.example.com")
	if err != nil {
		t.Fatal(err)
	}
	if changed {
