...
func TestMenuHeaderBoxTruncatesLongRows(t *testing.T) {
	line := boxLine("VPS    198.51.100.10  very-long-domain-name-that-would-otherwise-break-the-box.example")
	if got := len([]rune(line)); got != menuBoxInnerWidth+2 {
...
func TestStatusRowsKeepDeployVersionAndTime(t *testing.T) {
	state := &State{
		Backend: BackendState{
			Host: "198.51.100.10", Domain: "wgmonitor.example", LastDeployedVersion: "v0.13.0-rc12", LastDeploy: "2026-05-21T12:34:56Z",
		},
		Agents: []AgentState{{
			Nickname: "client-a", Host: "192.168.0.1", Port: 22, LastDeployedVersion: "v0.13.0-rc12", LastDeploy: "2026-05-21T12:40:00Z",
		}},
	}
	...
	for _, want := range []string{
		"VPS    198.51.100.10  wgmonitor.example",
		"installed v0.13.0-rc12 at 2026-05-21 12:34",
		"v0.13.0-rc12",
		"Router 192.168.0.1:22  client-a",
...
func TestStatusRowsUseReadableEndpointForAWGMOnlyRouters(t *testing.T) {
	...
	if !strings.Contains(rows, "Router AWGM  client-g") {
...
func TestStatusRowsUseReadableEndpointForMetadataGapRouters(t *testing.T) {
	...
	rows := strings.Join(menuStatusRows(state), "\n")
	if !strings.Contains(rows, "Router metadata-gap  client-g") {
...
func TestAgentSSHEndpointLabelHidesEmptyPort(t *testing.T) {
	...
	if got := agentSSHEndpointLabel(AgentState{Host: "192.168.0.1", Port: 222}); got != "192.168.0.1:222" {
...
func TestAgentChoiceLabelUsesReadableEndpoint(t *testing.T) {
	cases := []struct {
		...
		{
			name: "ssh",
			ag:   AgentState{Nickname: "testkeen", Host: "192.168.0.1", Port: 222},
			want: "testkeen (192.168.0.1:222)",
		},
	}
...
func TestKnownRouterLineUsesReadableAWGMAndSSHLabels(t *testing.T) {
	...
	for _, want := range []string{"[1] client-g", "mode=metadata-gap", "awgm=-", "ssh=-"} {
...
func TestInstalledStatusRowsAreDetectedForHighlight(t *testing.T) {
	...
	if isInstalledStatusRow("Router 192.168.0.1:222  testkeen") {
