...
### DEPLOY-34 — INFO — `Documentation=https://github.com/Jkaotlic/wg-monitor`
`wg-monitor-backend.service:3`
Verify this URL is correct — repo owner per release.yml is `${{ github.repository_owner }}` (dynamic), but unit hard-codes `user`. If repo gets transferred / forked org, this becomes stale.
